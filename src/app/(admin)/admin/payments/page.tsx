          ...
          <>
            ...
            <div className="md:hidden divide-y" style={{ borderColor: 'var(--nafa-gray-100)' }}>
              {filtered.map((t, i) => {
                const net = Math.round(t.subtotal * 10 / 11);
                return (
                  <motion.div key={t.fullId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                    className="p-4 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold nafa-mono" style={{ color: 'var(--nafa-orange)' }}>{formatOrderId(t.id)}</span>
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${t.status === 'completed' ? 'bg-green-50 text-green-700' : t.status === 'cancelled' ? 'bg-red-50 text-red-600' : 'bg-yellow-50 text-yellow-700'}`}>
                        {t.status === 'completed' ? 'Complété' : t.status === 'cancelled' ? 'Annulé' : 'En attente'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium" style={{ color: 'var(--nafa-black)' }}>{t.vendor}</span>
                      <span className="text-xs" style={{ color: 'var(--nafa-gray-400)' }}>{new Date(t.date).toLocaleDateString('fr-FR')}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs" style={{ color: 'var(--nafa-gray-700)' }}>Total : <span className="font-bold nafa-mono" style={{ color: 'var(--nafa-black)' }}>{formatCurrency(t.gross, 'FCFA')}</span></span>
                      <span className="text-xs" style={{ color: 'var(--nafa-gray-700)' }}>Net vendeur : <span className="font-bold nafa-mono" style={{ color: 'var(--nafa-green)' }}>{formatCurrency(net, 'FCFA')}</span></span>
                    </div>
                  </motion.div>
                );
              })}
            ...
            <div className="md:hidden divide-y" style={{ borderColor: 'var(--nafa-gray-100)' }}>
              {filtered.map((w, i) => (
                <motion.div key={w.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  className="p-4 flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold" style={{ color: 'var(--nafa-black)' }}>{w.vendor}</span>
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                      w.status === 'completed' ? 'bg-green-50 text-green-700'
                      : w.status === 'failed' ? 'bg-red-50 text-red-600'
                      : 'bg-yellow-50 text-yellow-700'}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold nafa-mono" style={{ color: 'var(--nafa-black)' }}>{formatCurrency(w.amount, 'FCFA')}</span>
                    <span className="text-xs capitalize" style={{ color: 'var(--nafa-gray-700)' }}>{w.method.replace('_', ' ')} · {new Date(w.date).toLocaleDateString('fr-FR')}</span>
                  </div>
                  {w.status === 'pending' && (
